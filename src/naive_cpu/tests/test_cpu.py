...
def test_cpu_cases(case: CpuCase):
    ...
    assert EBREAK_RE.search(sim_output), "Simulator output did not terminate with an ebreak."

    observed = _collect_registers(sim_output)
    for reg, expected in case.expected_regs.items():
        assert reg in observed, f"Register x{reg} was never written in case {case.name}."
        assert (
            observed[reg] == expected
