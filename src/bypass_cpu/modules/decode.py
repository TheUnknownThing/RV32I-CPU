"""Decode helpers for the Naive RV32I CPU."""
...
def _decode_instruction(instr: Value) -> Value:
    ...
    imm_branch = _sign_extend_bits(
        concat(instr[31:31], instr[7:7], instr[25:30], instr[8:11], Bits(1)(0)),
        13,
    )
    imm_jump = _sign_extend_bits(
        concat(instr[31:31], instr[12:19], instr[20:20], instr[21:30], Bits(1)(0)),
        21,
    )
    imm_u = concat(instr[12:31], Bits(12)(0))
    ...
    is_branch = opcode == Bits(7)(OPCODE_BRANCH)
    is_jal = opcode == Bits(7)(OPCODE_JAL)
    is_jalr = opcode == Bits(7)(OPCODE_JALR)
    is_auipc = opcode == Bits(7)(OPCODE_AUIPC)
    is_lui = opcode == Bits(7)(OPCODE_LUI)
    ...
    is_beq = is_branch & funct3_eq[0b000]
    is_bne = is_branch & funct3_eq[0b001]
    is_blt = is_branch & funct3_eq[0b100]
    is_bge = is_branch & funct3_eq[0b101]
    is_bltu = is_branch & funct3_eq[0b110]
    is_bgeu = is_branch & funct3_eq[0b111]
    ...
    with Condition(~supported):
        log("naive-decode | unsupported instruction opcode=0x{:x}", opcode)
        assume(Bits(1)(0))
    ...
    imm = is_branch.select(imm_branch, imm)
    imm = is_jal.select(imm_jump, imm)
    imm = (is_lui | is_auipc).select(imm_u, imm)

    rs2_is_source = is_op | is_store | is_branch
    rs1_is_source = Bits(1)(1)
    rs1_is_source = (is_lui | is_auipc | is_jal).select(Bits(1)(0), rs1_is_source)
    ...
    # Keep Select1Hot happy
    op_select = is_ebreak.select(mask(AluOp.ADD), op_select)
    op_select = (is_load | is_store | is_branch | is_jalr | is_jal | is_auipc | is_lui).select(mask(AluOp.ADD), op_select)
    ...
    branch_cond = is_bge.select(mask_branch(BranchCond.GE), branch_cond)
