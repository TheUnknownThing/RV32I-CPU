// Licensed under ucb-bar/riscv-benchmarks
...
static const int32_t input_data1[DATA_SIZE] =
{
...
static const int32_t input_data2[DATA_SIZE] =
{
...
static const int32_t verify_data[DATA_SIZE] =
{
...
static int32_t multiply(int32_t lhs, int32_t rhs) {
  uint32_t multiplicand = (uint32_t)(lhs < 0 ? -lhs : lhs);
  uint32_t multiplier = (uint32_t)(rhs < 0 ? -rhs : rhs);
  uint32_t result = 0;
...
int main(void) {
  ...
  for (size_t i = 0; i < DATA_SIZE; ++i) {
    const int32_t product = multiply(input_data1[i], input_data2[i]);
    checksum ^= (uint32_t)product;
    if (product != verify_data[i]) {
      ++mismatches;
    }
  }

  test_report(0, checksum);
  test_report(1, mismatches);

  if (mismatches == 0U) {
    test_pass();
  }
  test_fail(mismatches);
}