...
class Decoder(Module):
    ...
    def build(
        ...
        log(
            "toy-decode | pc=0x{:08x} rd=x{:02} rs1=x{:02} rs2=x{:02} load={} store={} br={}",
            pc_value,
            inst.rd,
            inst.rs1,
            inst.rs2,
...
def _decode_instruction(instr: Value) -> Value:
    ...
    op_select = (is_load | is_store | is_branch).select(mask(AluOp.ADD), op_select)
    ...
    log(
        "toy-decode | rd=x{:02} rs1=x{:02} rs2=x{:02} load={} store={} branch={} memw={} uns={} op=0x{:03x}",
        rd_value,
        rs1,
        rs2,
        is_load,
