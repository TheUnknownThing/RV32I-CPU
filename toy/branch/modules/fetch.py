...
class Fetcher(Module):
    """Maintains the program counter and handles branch redirects."""
    ...
    def build(
        ...
        with Condition(do_fetch):
            log("toy-fetch  | fetch | pc=0x{:08x} busy={}", pc_value, on_branch[0])
        ...
        with Condition(~do_fetch):
            log("toy-fetch  | not fetch | pc=0x{:08x} busy={}", pc_value, on_branch[0])
