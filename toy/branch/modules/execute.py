...
class Executor(Module):
    ...
    def build(
        ...
        rs1_value = reg_file[inst.rs1]
        rs2_value = reg_file[inst.rs2]
        result, store_value = self._execute_alu(inst, rs1_value, rs2_value)
        ...
        with Condition(inst.is_branch):
            ...
            branch_target_value = (pc_value.bitcast(Int(32)) + inst.imm.bitcast(Int(32))).bitcast(Bits(32))
            ...
            log(
                "toy-branch | pc:0x{:08x} rs1=x{:02} rs2=x{:02} taken={} target=0x{:08x}",
                pc_value,
                inst.rs1,
                inst.rs2,
                branch_taken,
    ...
    def _execute_alu(self, inst: Value, rs1_value: Value, rs2_value: Value):
        op_a = rs1_value
